        ...
        if (!((headgear _x) in _masks || (goggles _x) in _masks || (uniform _x) in _masks)) then {
            if (count _sPos > 1 && {_distance < 30}) then {
                _text = switch (true) do {
                    case (_x in (units group player) && playerSide isEqualTo civilian): {format["<t color='#00B66E'>%1</t>",(_x getVariable ["realname",name _x])];};
                    ...
                    case ((side _x isEqualTo civilian) && (FETCH_CONST(life_adminlevel) > 0)): {format["<t color='#04B404'>%1</t><br/><t color='#04B404'>Admin</t>",_x getVariable ["realname",name _x]]};
                    default {
